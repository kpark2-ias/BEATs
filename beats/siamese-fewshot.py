...
def compute_metrics(df, category, model_name, shot):
    ...
    df_concated.to_csv(f"results/siamese_fewshot_results/{model_name}/{mode}/result_shot_{shot}_argmax.csv", index=False)
    ...
    df_concated = pd.concat([df_concated, df2], axis=1)
    df_concated.to_csv(f"results/siamese_fewshot_results/{model_name}/{mode}/result_shot_{shot}_threshold.csv", index=False)

def main(model_name, labels, input_dir, shot, mode):
    ...
    category = []
    for cat_q in q:
        category.append(cat_q)
        for item in q[cat_q]:
            output = [] 
            for idx, cat_m in enumerate(mu):
                
                output.append(torch.flatten(model(mu[cat_m].reshape((1, -1)),item.reshape((1,-1)))))
                                         
            output = torch.stack(output)
            predicted_prob.append(torch.flatten(output).tolist())
            binary = torch.where(output > threshold, 1, 0)
            binary = torch.flatten(binary).tolist()
            predicted_binary.append(binary)
            ###multi-label###
            
            result_max_sim = torch.argmax(output).item()
    ...
    result.to_csv(f"results/siamese_fewshot_results/{model_name}/{mode}/result_shot_{shot}_multilabel_output.csv", index=False)
    
    compute_metrics(df, category, model_name, shot)
...
if __name__ == "__main__": 
    ...
    parser.add_argument('-l','--labels', default='few-shot.csv')
    ...
    for i in range(1, 6):
        for model_name in ["BEATs", "AST", "Distil-AST"]:
            dir_path = f"results/siamese_fewshot_results/{model_name}"
            if not os.path.exists(dir_path):
                os.mkdir(dir_path)
            for mode in ["diff"]:
                sub_dir = dir_path+f"/{mode}"
